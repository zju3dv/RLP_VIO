...
class PVIOPcVisualizer : public nanovis::NanoVis {
  ...
  public:
    PVIOPcVisualizer(const std::string &data_path, const std::string &yaml_path,const std::string &save_path) {
        // [1] data
        ...
        std::cout << "algorithm works" << std::endl;
        // [4] gui
        feature_tracker_painter = std::make_unique<OpenCvPainter>(feature_tracker_image);
        
        //ignoring the visulization part
#ifdef VISUALIZE_FLAG
        {
        ...
        std::cout << "painter works" << std::endl;
        ...
        output_writer = std::make_unique<TumOutputWriter>(save_path);
        std::cout << "output_writer works" << std::endl;
...
int main(int argc, const char *argv[]) {
    ...
    PVIOPcVisualizer visualizer(argv[1], argv[2],argv[3]);
#ifdef VISUALIZE_FLAG
    visualizer.show();
    nanovis::main(1);
#else
    while(visualizer.headless_step()){};
    visualizer.shutdown();
#endif
    return 0;
}
