...
matrix<3> find_rotation_matrix(const std::vector<vector<3>> &points1, const std::vector<vector<3>> &points2, double threshold, double confidence, size_t max_iteration, int seed) {
    std::vector<char> _;
    return find_rotation_matrix(points1, points2, _, threshold, confidence, max_iteration, seed);
}
...
matrix<3> find_rotation_matrix(const std::vector<vector<3>> &points1, const std::vector<vector<3>> &points2, std::vector<char> &inlier_mask, double threshold, double confidence, size_t max_iteration, int seed) {
    struct RotationSolver {
        matrix<3> operator()(const std::array<vector<3>, 2> &samples1, const std::array<vector<3>, 2> &samples2) const {
            return solve_rotation_2pt(samples1, samples2);
        }
    };
    struct RotationEvaluator {
        const matrix<3> &R;
        RotationEvaluator(const matrix<3> &R) :
            R(R) {
        }
    ...
    Ransac<2, matrix<3>, RotationSolver, RotationEvaluator> ransac(t2 * threshold * threshold, confidence, max_iteration, seed);
