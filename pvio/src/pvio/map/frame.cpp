...
Frame::Frame(const Frame &frame, const construct_by_frame_t &construct_by_frame) :
    Flagged(frame), Identifiable(frame), map(nullptr) {
}
...
void Frame::detect_keypoints(Config *config) {
    ...
    // std::ofstream static_check("/home/zhouxin/projects/PVIO/static_check.txt",std::ios::app);
    // static_check << id() << " " << int(flag(FrameFlag::FT_NO_TRANSLATION) == true) << " " << motion.v.transpose() << std::endl;
    // static_check.close(); 
...
void Frame::track_keypoints(Frame *next_frame, Config *config) {
    ...
    std::vector<vector<3>> bearings(keypoints.size());

    for (size_t i = 0; i < keypoints.size(); ++i) {
        curr_keypoints[i] = apply_k(keypoints[i], K);
        bearings[i] = keypoints[i].homogeneous().normalized();
    }
    ...
    for (size_t i = 0; i < curr_keypoints.size(); ++i) {
        ...
        vector<2> next_keypoint_2d = remove_k(next_keypoints[i], next_frame->K);
        vector<3> next_keypoint_3d = vector<3>{next_keypoint_2d[0], next_keypoint_2d[1], 1}.normalized();
        next_keypoints_h.push_back(next_keypoint_3d.hnormalized());
        next_bearings.push_back(next_keypoint_3d);
    }

    if(next_keypoints_h.size() > 10){
        ...
        matrix<3> R = find_rotation_matrix(bearings, next_bearings, mask, M_PI / 18.0); // TODO: make configurable
        ...
        if (misalignment < 0.05) { // TODO: make configurable
            next_frame->flag(FrameFlag::FT_NO_TRANSLATION) = true;
        }
    }
