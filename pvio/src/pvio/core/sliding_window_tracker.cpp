...
bool SlidingWindowTracker::track() {
    ...
    if(frame->flag(FrameFlag::FT_NO_TRANSLATION)){
        set_fix_virtual_point(frame.get());
        //analyze_track_type_in_map(map.get());
    }else{
        for (size_t i = 0; i < frame->keypoint_num(); ++i) {
            Track *track = frame->get_track(i);
            if (!track) continue;
            if (track->flag(TrackFlag::TF_VIRTUAL)){
                track->flag(TrackFlag::TF_VIRTUAL) = false;
                track->flag(TrackFlag::TF_PLANE) = false;
            }
            if (track->flag(TrackFlag::TF_VALID)){
                track->flag(TrackFlag::TF_FIXED) = false;
                continue;
            }
            track->triangulate(); //triangulate the vritual point(done)
        }
    }
...
void SlidingWindowTracker::set_fix_virtual_point(Frame *frame){
    Frame *last_frame = map->last_frame();
    for (size_t i = 0; i < frame->keypoint_num(); ++i) {
        Track *track = frame->get_track(i);
        if (!track) continue;
        if (track->first_frame()->flag(FrameFlag::FT_NO_TRANSLATION) && !track->flag(TrackFlag::TF_VALID)){
            track->flag(TrackFlag::TF_VIRTUAL) = true;
            continue;
        }
        if (track->flag(TrackFlag::TF_VALID)){
            if(track->keypoint_num() > 7){
                track->flag(TrackFlag::TF_FIXED) = true;
            }
        }
    }
    return;
