...
void PlaneExtractor::extend_planes_and_cast_plane_points(double extend_rpe_ratio) {
    ...
    for (size_t i = 0; i < map->track_num(); ++i) {
        ...
        //if (!track->flag(TrackFlag::TF_VALID) && !track->flag(TrackFlag::TF_VIRTUAL)) continue; // it seems better to skip these invalid tracks

        if (!track->flag(TrackFlag::TF_VALID)) continue; // it seems better to skip these invalid tracks

        double rpe_before_project = 99999999;
        if(!track->flag(TrackFlag::TF_VIRTUAL)){
            vector<3> point = track->get_landmark_point();
            rpe_before_project = compute_reprojection_error(map, track, point);
        }
